...
def add_hat(img,hat_img):
    ...
    cv2.imwrite("./tools/hat_alpha.jpg",a)
    ...
    # dlib人脸关键点检测器
    predictor_path = "./tools/shape_predictor_5_face_landmarks.dat"
    ...
    if len(dets)>0:  
        for d in dets:
            ...
            cv2.imwrite("./tools/bg.jpg",bg)
            # cv2.imshow("image",img)
            # cv2.waitKey()

            # 提取帽子区域
            hat = cv2.bitwise_and(resized_hat,resized_hat,mask = mask)
            cv2.imwrite("./tools/hat.jpg",hat)
...
def fun(listfiles):
    ...
    hat_img = cv2.imread("./tools/hat2.png",-1)
    for pathfile in listfiles:
        ...
        newfileName = os.path.splitext(fileName)[0] + '_1' + os.path.splitext(fileName)[1]
    ...
    for filename in files:
        if not os.path.isdir(filename):
            try:
                ...
                if (extension.lower() == ".png" or extension.lower() == ".jpg") and os.path.splitext(filename)[0][-2:] != '_1':
